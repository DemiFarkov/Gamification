...
const TestsStatisticss = () => {
  ...
  function filter(el) {
    let good = true;
    if (selectEmployee !== "") {
      if (el.full_name !== selectEmployee) {
        good = false;
      }
    }
    if (selectTest !== "") {
      if (el.test__name !== selectTest) {
        good = false;
      }
    }
    if (selectTheme !== "") {
      if (el.test__theme__name !== selectTheme) {
        good = false;
      }
    }
    if (selectStatus !== "") {
      if (el.result !== selectStatus) {
        good = false;
      }
    }
    if (lastAttempt == true) {
  ...
  useEffect(() => {
    if (arrFilter) {
      ...
      arrFilter.map(
        (el) => (
          (score = score + el.total_score),
          (maxScore = maxScore + el.max_score),
          (acoin = acoin + el.test_acoin_reward),
          (exp = exp + el.test_experience_points),
          el.result == "Passed" && peoplePassed++
        )
      );

      setAverageScore(((peoplePassed / arrFilter.length) * 100).toFixed(0));
      setAcoins(acoin);
      setExp(exp);
    }
  }, [arrFilter]);
  useEffect(() => {
    if (sortMode.length > 0) {
      ...
      if (sortMode[0] == "date") {
        let sortArr = structuredClone(arrFilter).sort((a, b) =>
          a.end_time.slice(0, a.end_time.indexOf(" ")).replaceAll("-", "") ==
          b.end_time.slice(0, b.end_time.indexOf(" ")).replaceAll("-", "")
            ? 0
            : a.end_time.slice(0, a.end_time.indexOf(" ")).replaceAll("-", "") >
              b.end_time.slice(0, b.end_time.indexOf(" ")).replaceAll("-", "")
            ? 1
                            ...
                            arrFilter.map((el, index) => (
                              ...
                              >
                                <AccordiontStatisticss
                                  ...
                                  test_attempt={el.id}
                                  key={index}
                                  allEmployee={allEmployee.statistics}
                                />
                              </CSSTransition>
                            ))}
                        </TransitionGroup>
